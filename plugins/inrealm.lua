...
function run(msg, matches)
    ...
    if matches[1] == '[Cc]reategroup' and matches[2] then
        group_name = matches[2]
        group_type = 'group'
        return create_group(msg)
    end

    if matches[1] == '[Cc]reaterealm' and matches[2] then
        group_name = matches[2]
        group_type = 'realm'
	...
	if matches[2] then if data[tostring(matches[2])] then
		...
		if matches[1] == '[Ss]etabout' and matches[2] then
			local target = matches[2]
		    local about = matches[3]
		    return set_description(msg, data, target, about)
		end
		if matches[1] == '[Ss]etrules' then
		    rules = matches[3]
			local target = matches[2]
		    return set_rules(msg, data, target)
		end
		if matches[1] == '[Ll]ock' then --group lock *
			local target = matches[2]
		...
		if matches[1] == '[Uu]nlock' then --group unlock *
			local target = matches[2]
		    if matches[3] == 'name' then
		...
		if matches[1] == '[Ss]ettings' and data[tostring(matches[2])]['settings'] then
			local target = matches[2]
		    return show_group_settings(msg, data, target)
		end

                if matches[1] == '[Ss]etname' and is_realm(msg) then
		...
		if matches[1] == '[Ss]etgpname' and is_admin(msg) then
    	...
    	if matches[1] == '[Hh]elp' and is_realm(msg) then
     		...
     		return help()
              ...
              if matches[1] == '[Ss]et' then
                ...
                if matches[1] == '[Kk]ill' and matches[2] == 'realm' then
                  if not is_admin(msg) then
                     return nil
                  end
                  if is_realm(msg) then
		...
		if matches[1] == '[Rr]emoveadmin' then
			if string.match(matches[2], '^%d+$') then
				local admin_id = matches[2]
